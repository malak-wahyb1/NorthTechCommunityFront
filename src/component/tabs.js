...
export default function BasicTabs() {
  ...
  useEffect(() => {
    axios
      .get(
        `https://northtechcommunity3.onrender.com/friend/request/${user._id}`
      )
      .then((response) => {
  ...
  useEffect(() => {
    axios
      .get(
        `https://northtechcommunity3.onrender.com/friend/accepted/${user._id}`
      )
      .then((response) => {
    ...
    <Box sx={{ width: "100%" }}>
      ...
      <TabPanel value={value} index={0}>
        {friend.length > 0 ? (
          ...
          <section className="acceptedFriend">
            <div className="unhotorized">
              <div className="unhotorizedcontainer">
                <div className="text">
                  {">_ "}
                  <Link to="">No request yet.</Link>
